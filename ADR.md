...
| 018 | Extensible Palette — Core Lock Replaced by Versioned Reference Set | Accepted, supersedes 012 |
...
**Status**: Accepted
**Date**: 2026-04-18
**Context**: PixelLab-generated sprites must be reviewed by the human before being wired into scenes or committed. Previously no formal gate existed — Claude could generate straight into `assets/` and the generated output would enter git before the user ever saw it. Parallel concern: image tokens are not free when Claude reads them inline, so the default review path should not force Claude to load every generated sprite into context.
**Decision**: (1) All PixelLab MCP outputs land in `assets/_pending/`, which is gitignored except for a `.gitkeep` so the folder travels with the repo. (2) Default review mode is **user opens the PNG manually** in Windows preview; Claude reports filename, prompt, and seed/settings and waits for approval. (3) Claude must flag `⚠ recommend inline review — reason: <one line>` and offer to display the image via Read when any of these trigger: first-in-series, new character/location/style, palette-sensitive, silhouette-critical at 32×32, visible artifacts, or visible drift from a committed sibling. (4) On approval Claude moves the file from `_pending/` to the canonical `assets/<category>/` path; on rejection it is regenerated or deleted. Nothing enters `assets/` or git until approved. Workflow codified in CLAUDE.md "Asset Review Workflow" section.
...
## ADR-018: Extensible Palette — Core Lock Replaced by Versioned Reference Set (Supersedes ADR-012)
**Status**: Accepted
**Date**: 2026-04-19
**Context**: ADR-012 locked the palette to exactly 10 colors (5 ENV + 3 UI + 1 BRIDGE + 1 BASE) and stated that adding colors required an ADR + GDD + palette.md version bump — a friction cost intended to prevent drift. In practice, hand-drawn tile art (ADR-017 pipeline) naturally produces real colors from real creative decisions, not from PixelLab generation. The interior bedroom tileset was drawn with a rose/mauve interior set (31 colors), warm creams (8), teals, blues, purples, and near-black mauves — colors that don't exist in the locked 10. The lock was designed to constrain AI generation, not to veto human-drawn assets.
**Decision**: The palette becomes a versioned, extensible reference set. (1) The original 10 colors (ADR-012) are retained as the **canonical core set** — their rules (ENV/UI/BRIDGE/BASE separation, awakening-arc reveal, per-screen budget) are unchanged. (2) Hand-drawn tile and art work may introduce new colors, which are back-registered into `palette.gpl` and `palette.md` as named **extended sections** (e.g., "Interior Bedroom Set"). (3) Each extension requires a `palette.md` version bump and a brief origin note (which asset the colors came from). (4) PixelLab enforcement prompt (palette.md §5) still enforces the canonical core ENV/UI colors for generated sprites — extended palette entries do not bleed into PixelLab calls. (5) "Locked" language is removed from palette.md; "versioned" replaces it.
**Rationale**: The original lock was anti-drift insurance for AI generation, not a veto on human creative decisions. Forcing hand-drawn tile colors to conform to 5 environment colors removes the artist's ability to design interior spaces with appropriate warmth or coolness. The set-separation rules (ENV vs UI vs BRIDGE) and the awakening-arc reveal mechanic are the architecturally important invariants — those survive. The hard count constraint does not.
**Consequences**: `palette.md` updated to v1.1 and annotated as extensible (not locked). `palette.gpl` gains new named color groups beneath the original 10. ADR-012 status updated to Superseded. Near-duplicate colors from quantization noise in exported PNGs should be collapsed before registration — palette entries should represent intentional color choices, not export artifacts. The palette.tres Godot resource (ADR-015) does not yet need updating; extended colors become Godot-exposed only when a scene or script actively references them.
...
- 2026-04-19 (Session 007): Interior bedroom tileset (InteriorTilesLITE_edited-export.png) introduced 80 unique colors not in the locked palette. ADR-018 supersedes ADR-012 — palette becomes an extensible versioned reference set. Core 10-color rules (set separation, awakening arc, PixelLab enforcement) unchanged. Extended colors from hand-drawn tiles back-registered as named sections in palette.gpl and palette.md.
