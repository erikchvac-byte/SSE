...
## Phase
Phase 1 scaffold in place — GDD v0.2.3. Palette resource, minimap UI, bedroom scaffold with wall collision, placeholder player CharacterBody2D, stat system (YouthUnitStats + PlayerState autoload), stat HUD with dual-register awakening reveal. Asset pipeline split per ADR-017: tiles/textures hand-drawn in Aseprite, sprites generated by PixelLab at 64×64 then redrawn at 32×32 in Aseprite. No real sprites yet; all visuals are palette-colored rects via _draw(). PixelLab balance: $9.81 (hard floor $9.50 per ADR-017).
